...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        token.id = user.id;
        token.role = (
          ((user as { role?: string }).role as string) || "owner"
        ) as "owner" | "manager" | "apprentice" | "admin";
      }
      return token;
    },
    async session({ session, token }) {
      if (session.user) {
        session.user.id = token.id as string;
        session.user.role = (token.role || "owner") as
          | "owner"
          | "manager"
          | "apprentice"
          | "admin";
  ...
  // FIX: debug disabled — was causing [next-auth][warn][DEBUG_ENABLED]
  // warning and adding 6-8 seconds to every auth request in dev
  debug: false,
};

export async function getDesignerFromSession(
  session: {
