...
import type { BodyGender } from "@/lib/body-measurement";
...
export default function ClientDetailPage() {
  ...
  /* ---- Compute plausibility warnings when client loads ---- */
  useEffect(() => {
    if (!client?.measurements || client.measurements.reviewedByDesigner) {
      setPlausibilityWarnings([]);
      return;
    }
    const m = client.measurements as Measurements;
    if (m.source !== "ai_scan" || !m.height) return;
    const gender = (client.gender || "female") as BodyGender;
  ...
  /* ---- Approve / adjust AI measurements ---- */
  const handleApproveMeasurements = async (approve: boolean) => {
    try {
      setApprovingMeasurements(true);
      const res = await fetch(`/api/clients/${clientId}/measurements`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          adjustments: Object.keys(reviewAdjustments).length > 0 ? reviewAdjustments : undefined,
          approve,
        }),
      });
      const json = await res.json();
      ...
      if (!json.success) {
        throw new Error(json.error || "Failed to approve measurements");
      }

      toast.success(approve ? "Measurements approved" : "Adjustments saved");
      setReviewMode(false);
      setReviewAdjustments({});
      fetchClient();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Failed to update measurements");
    } finally {
      setApprovingMeasurements(false);
    }
  };
      ...
      <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-6 sm:px-6">
        ...
        >
          <GlassCard padding="lg">
            ...
            {measurements ? (
              <>
                {/* AI Review Banner — shows for unreviewed AI scans */}
                {measurements.source === "ai_scan" && !measurements.reviewedByDesigner && (
                  <div className="mb-4 rounded-xl border border-amber-300/40 bg-amber-50/60 p-3.5">
                    <div className="flex items-start gap-3">
                      <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-500" />
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-amber-800">
                          AI Measurements — Please Review
                        </p>
                        <p className="mt-0.5 text-xs text-amber-700/70">
                          These measurements were generated by AI. Review and adjust before using for production.
                        </p>
                          ...
                          <div className="mt-2 space-y-1">
                            {plausibilityWarnings.map((w, i) => (
                              ...
                              >
                                ...
                                <span><strong className="capitalize">{w.field}</strong>: {w.message}</span>
                              </div>
                            ))}
                          </div>
                        )}
                        <div className="mt-3 flex gap-2">
                          <Button
                            ...
                            onClick={() => {
                              if (!reviewMode) {
                                // Pre-fill adjustments with current values
                                const adj: Record<string, number> = {};
                                MEASUREMENT_TYPES.forEach((t) => {
                                  const v = measurements[t.key as keyof Measurements] as number | undefined;
                                  if (v) adj[t.key] = v;
                                });
                                setReviewAdjustments(adj);
                              }
                              setReviewMode(!reviewMode);
                            }}
                          >
                            <Edit className="h-3.5 w-3.5" />
                            {reviewMode ? "Cancel Editing" : "Review & Adjust"}
                          </Button>
                          {!reviewMode && (
                            <Button
                              size="sm"
                              ...
                              onClick={() => handleApproveMeasurements(true)}
                              loading={approvingMeasurements}
                            >
                ...
                {/* Approved badge */}
                {measurements.reviewedByDesigner && (
                  <div className="mb-4 flex items-center gap-2 rounded-lg bg-emerald-50/60 px-3 py-2">
                    <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                    <span className="text-xs font-medium text-emerald-700">
                      Reviewed & approved by designer
                      {measurements.reviewedAt && (
                        <span className="ml-1 text-emerald-600/60">
                          ({formatDate(measurements.reviewedAt)})
                        </span>
                      )}
                    </span>
                  </div>
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                      {MEASUREMENT_TYPES.map((type) => {
                        const value = reviewAdjustments[type.key];
                        if (!value && !(measurements[type.key as keyof Measurements] as number)) return null;
                        const current = value || (measurements[type.key as keyof Measurements] as number) || 0;
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {warning && (
                                <AlertTriangle
                                  className={cn(
                                    "h-3 w-3",
                                    warning.severity === "critical" ? "text-red-500" : "text-amber-500"
                                  )}
                                />
                              )}
                            </div>
                              ...
                              <button
                                onClick={() =>
                                  setReviewAdjustments((prev) => ({
                                    ...prev,
                                    [type.key]: Math.max(0, (prev[type.key] || current) - 0.5),
                                  }))
                                }
                                className="flex h-7 w-7 items-center justify-center rounded-lg border border-[#1A1A2E]/10 bg-white/60 text-[#1A1A2E]/60 active:bg-white/80"
                              >
                                <ChevronDown className="h-3.5 w-3.5" />
                              </button>
                              <span className="min-w-[3rem] text-center text-sm font-semibold text-[#1A1A2E]">
                                {(reviewAdjustments[type.key] || current).toFixed(1)}
                              </span>
                              <button
                                onClick={() =>
                                  setReviewAdjustments((prev) => ({
                                    ...prev,
                                    [type.key]: (prev[type.key] || current) + 0.5,
                                  }))
                                }
                                className="flex h-7 w-7 items-center justify-center rounded-lg border border-[#1A1A2E]/10 bg-white/60 text-[#1A1A2E]/60 active:bg-white/80"
                              >
                    ...
                    <div className="mt-4 flex gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => { setReviewMode(false); setReviewAdjustments({}); }}
                      >
                        Cancel
                      </Button>
                    </div>
